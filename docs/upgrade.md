...
rm forgeloop.specify-old.md
rm forgeloop.plan-v1.md
```
...
rm forgeloop.old-command-name.md
...
Once you've run `specify init`, the slash commands (like `/forgeloop.specify`, `/forgeloop.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/forgeloop.constitution` or another command to verify everything works
